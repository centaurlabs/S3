...
extension S3 {
    ...
    /// Get list of objects
    public func list(bucket: String, region: Region? = nil, headers: [String: String] = [:], listInfo: ListInfo = .default, on container: Container) throws -> Future<BucketResults> {
        let region = region ?? signer.config.region
        guard let baseUrl = URL(string: region.hostUrlString(bucket: bucket)), let host = baseUrl.host,
            var components = URLComponents(string: baseUrl.absoluteString) else {
            throw S3.Error.invalidUrl
        }
        components.queryItems = listInfo.urlQueryItems
        guard let url = components.url else {
            throw S3.Error.invalidUrl
        }
        var headers = headers
